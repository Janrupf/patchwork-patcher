...
public class CommandlineParser<T> {
	...
	// cache them
	private static final Map<Class<?>, MethodHandle> CACHED_TYPE_MAPPER_CONSTRUCTORS =
			new HashMap<>();
	 ...
	 * generate a full help message use {@link CommandlineParser#generateHelpMessage(String, String,
	...
	private void buildAccessors() throws CommandlineException {
			...
			if((parameterAnnotation = field.getAnnotation(Parameter.class)) != null) {
				if(unorderedParameters.containsKey(parameterAnnotation.position())) {
					throw new CommandlineException(
							"Duplicated parameter position " + parameterAnnotation.position());
				}
				...
				if(parameterAnnotation.position() == -1) {
					if(remainingAccessor != null) {
						throw new CommandlineException(
								"Duplicated remaining accessor, already have " +
								remainingAccessor.names()[0] + " and tried to add " +
				...
				unorderedParameters.put(parameterAnnotation.position(),
						new FieldAccessor(getTypeMapper(parameterAnnotation.typeMapper(), field),
								new String[] { parameterAnnotation.name() },
								parameterAnnotation.description(),
								parameterAnnotation.required()));
				...
				flagAccessors.add(
						new FieldAccessor(getTypeMapper(flagAnnotation.typeMapper(), field),
		...
		for(Map.Entry<Integer, FieldAccessor> entry : unorderedParameters.entrySet()
															  .stream()
			...
			if(entry.getKey() != index) {
				...
				throw new CommandlineException("Parameter index error, position gap? Expected " +
											   entry.getKey() + " for parameter " +
											   Arrays.toString(entry.getValue().names()) +
											   " but got " + index);
			}

			if(index != 0 && !parameterAccessors.get(index - 1).required() &&
				...
				throw new CommandlineException(
						"Parameter " + Arrays.toString(entry.getValue().names()) + " which is "
	...
	private TypeMapper getTypeMapper(Class<? extends TypeMapper> annotatedClass, Field f)
		...
		if(annotatedClass != TypeMapper.NullTypeMapper.class) {
			...
			try {
				if(CACHED_TYPE_MAPPER_CONSTRUCTORS.containsKey(annotatedClass)) {
					...
					// lookup
					return (TypeMapper) CACHED_TYPE_MAPPER_CONSTRUCTORS.get(annotatedClass)
							.invoke(argumentHolder, f);
				}

				// Search for a constructor matching
				MethodHandle constructor = METHOD_LOOKUP.findConstructor(annotatedClass,
						MethodType.methodType(void.class, Object.class, Field.class));
				...
				throw new CommandlineException(
						"Creating custom type mapper " + annotatedClass.getName() +
								" failed due to missing a constructor accepting a Field as the only parameter",
						e);
			} catch(IllegalAccessException e) {
				throw new CommandlineException(
						"Creating custom type mapper " + annotatedClass.getName() +
								" failed due to bad access rights (invisible constructor?)",
						e);
			} catch(Throwable t) {
				throw new CommandlineException("Creating custom type mapper " +
													   annotatedClass.getName() +
													   " failed due to error in constructor",
						t);
			}
			...
			try {
				...
				throw new CommandlineException(
						"Error while creating default type mapper for type " +
	...
	private void processArgs() throws CommandlineException {
					...
					for(String accessorName : accessor.names()) {
						...
						if(accessorName.equals(name)) {
							...
							if(accessor.acceptsValue()) {
								...
								if(value != null) {
									parseError = "Unexpected value '" + value + "' for flag " +
												 currentArg;
									return;
		...
		if(parameterIndex < parameterAccessors.size() &&
				parameterAccessors.get(parameterIndex).required()) {
			FieldAccessor accessor = parameterAccessors.get(parameterIndex);
	 ...
	 */
	public String generateHelpMessage(String executableName,
			String programName,
			String intro,
			String outro,
			boolean colors) {
		...
		if(!parameterAccessors.isEmpty()) {
			...
			for(FieldAccessor accessor : parameterAccessors) {
				...
				// - "[" & "]" for optional ones
				buffer.append("  ")
						.append(color((accessor.required() ? "<" : "[") + accessor.names()[0] +
											  (accessor.required() ? ">" : "]"),
								Ansi.Color.GREEN,
								true,
								colors))
		...
		if(!flagAccessors.isEmpty()) {
			...
			buffer.append(color("\nAvailable flags", Ansi.Color.MAGENTA, true, colors))
					.append(":\n");
			for(FieldAccessor accessor : flagAccessors) {
				String[] names = accessor.names();
		...
		if(parseError != null) {
			...
			buffer.append("\n\n")
					.append("Commandline error: ")
					.append(color(parseError, Ansi.Color.RED, true, colors));
		}
	...
	private void appendDescription(StringBuilder buffer,
	...
	public String generateSignature(String executableName) {
		...
		if(!parameterAccessors.isEmpty()) {
			for(FieldAccessor accessor : parameterAccessors) {
				buffer.append(accessor.required() ? " <" : " [")
						.append(accessor.names()[0])
						.append(accessor.required() ? ">" : "]");
			}
		}
